        ...
        <Dialog.Popup className="fixed left-1/2 top-1/2 z-50 w-[min(92vw,32rem)] -translate-x-1/2 -translate-y-1/2 rounded-lg border border-border bg-card shadow-lg p-5 outline-none data-[starting-style]:opacity-0 data-[ending-style]:opacity-0 data-[starting-style]:scale-95 data-[ending-style]:scale-95 transition-all">
          ...
          {invoice ? (
            <div className="mt-4 space-y-4 text-sm">
              <section>
                <h3 className="font-medium mb-2">{t("invoice.sectionInvoice", { defaultValue: "发票信息" })}</h3>
              ...
              <section>
                <h3 className="font-medium mb-2">{t("invoice.sectionOrder", { defaultValue: "关联订单" })}</h3>
                {order ? (
                  <>
              ...
              <section>
                <h3 className="font-medium mb-2">{t("invoice.sectionProduct", { defaultValue: "产品规格" })}</h3>
                {product ? (
                  <>
          ...
          <div className="mt-5 flex justify-end">
            <button
              type="button"
              onClick={onClose}
              className="px-3 py-1.5 rounded text-sm font-medium bg-primary text-primary-foreground hover:opacity-90"
              autoFocus
            >
