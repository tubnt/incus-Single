...
function CreateTokenForm({ onCreated }: { onCreated: (token: string) => void }) {
  ...
  return (
    <div className="border border-border rounded-lg bg-card p-4 mb-6">
      ...
      >
        {mutation.isPending ? t("apiToken.creating", { defaultValue: "创建中..." }) : t("apiToken.createButton", { defaultValue: "创建" })}
      </button>
    </div>
  );
}
    ...
    <div className="border border-border rounded-lg bg-card p-4 flex items-center justify-between">
      <div>
        ...
        <div className="text-xs text-muted-foreground mt-1">
          ...
          {token.last_used_at && ` · ${t("apiToken.lastUsed", { defaultValue: "最后使用" })} ${new Date(token.last_used_at).toLocaleString()}`}
        </div>
      </div>
      <button
        onClick={async () => {
