...
preflight_check() {
  ...
  # 识别当前节点
  LOCAL_NODE_DEF="$(get_local_node)" || {
    log_error "无法识别当前节点（未匹配到管理网 IP），请检查网络配置"
    exit 1
  }
  parse_node "$LOCAL_NODE_DEF"
  LOCAL_NAME="$NODE_NAME"
  LOCAL_MGMT_IP="$NODE_MGMT_IP"
  ...
  for node in "${CLUSTER_NODES[@]}"; do
    parse_node "$node"
    if [[ "$NODE_MGMT_IP" == "$LOCAL_MGMT_IP" ]]; then
      continue
    fi
    if ! ping -c 1 -W 2 "$NODE_MGMT_IP" &>/dev/null; then
      log_warn "无法 ping 通 ${NODE_NAME} (${NODE_MGMT_IP})"
      fail=$((fail + 1))
    fi
  done
  if [[ $fail -gt 0 ]]; then
